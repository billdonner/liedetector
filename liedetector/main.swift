//
//  main.swift
//  veracitatorr on 6/25/23.
//

//Step 3: liedetector executes a script file from Prepper, sending each prompt to (another) Chatbot and generates a single output file of JSON data which is read by Blender."
...
struct LieDetector: ParsableCommand {
  ...
  func run() throws {
    print(">Liedetector Command Line: \(CommandLine.arguments)")
    print(">Liedetector is running at \(Date())")
    
    let apiKey = try getAPIKey()
    
    print(">Liedetector Using apikey: " + apiKey)
    ...
    do {
      ...
      if error as? PumpingErrors == PumpingErrors.reachedMaxLimit {
        print("\n>Liedetector reached max limit of \(ctx.max) prompts sent to the AI")
        print(">Liedetector Exiting Normally - Pumped:\(ctx.pumpCount)" + " Bad Json: \( ctx.badJsonCount)" + " Network Issues: \(ctx.networkGlitches)\n")
        LieDetector.exit()
      } else
      if error as? PumpingErrors == PumpingErrors.reachedEndOfScript {
        print("\n>Liedetector reached end of input script \(ctx.pumpCount) prompts sent to the AI")
        print(">Liedetector Exiting Normally - Pumped:\(ctx.pumpCount)" + " Bad Json: \( ctx.badJsonCount)" + " Network Issues: \(ctx.networkGlitches)\n")
        LieDetector.exit()
      }
      else {
